...
-- 1. Отключаем мусорные правила маршрутизации в чат "Счета на оплату"
UPDATE public.notification_routing_rules
SET is_enabled = false
WHERE event_type IN ('invoice.payment_changed', 'invoice.overdue');

-- 2. Переписываем триггер: invoice.created уходит только когда
-- статус заявки = "Счёт в Бухгалтерии" И есть invoice_number
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    -- invoice.created уходит в чат "Счета на оплату" ТОЛЬКО когда
    -- статус заявки = "Счёт в Бухгалтерии" И есть номер счёта.
    -- Триггер: либо статус только что стал таким, либо номер счёта появился при таком статусе.
    invoice_should_fire := (
    ...
    -- payment_status и overdue больше НЕ уходят в чат счетов
    -- (правила маршрутизации отключены выше)
