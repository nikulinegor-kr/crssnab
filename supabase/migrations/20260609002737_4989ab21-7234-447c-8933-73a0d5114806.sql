ALTER TABLE public.requests ADD COLUMN IF NOT EXISTS received_by text;
...
BEGIN
  ...
  IF (TG_OP = 'UPDATE') THEN
    ...
    IF (OLD.received_by IS DISTINCT FROM NEW.received_by AND NEW.received_by IS NOT NULL) THEN
      INSERT INTO public.request_activities (request_id, organization_id, user_id, action, field_name, old_value, new_value, description, snapshot)
      VALUES (NEW.id, NEW.organization_id, auth.uid(), 'received_confirmed', 'received_by',
        COALESCE(OLD.received_by,''), NEW.received_by,
        'Приёмку ТМЦ осуществил: ' || NEW.received_by, to_jsonb(OLD));
    END IF;
