...
DECLARE
  ...
  obj_name text;
BEGIN
  SELECT name INTO obj_name FROM public.objects WHERE id = r.object_id;
...
-- Replace old builder to call v2 (backward compatibility), and update trigger to use v2 directly
CREATE OR REPLACE FUNCTION public.build_incoming_message(r requests)
RETURNS text
LANGUAGE sql
STABLE SECURITY DEFINER
SET search_path TO 'public'
AS $$ SELECT public.build_incoming_message_v2(r) $$;