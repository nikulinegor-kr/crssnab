...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    IF OLD.status IS DISTINCT FROM NEW.status THEN
      -- On the assignment transition use the rich "assigned" template so the
      -- supply group sees a complete card. Editing the incoming-group message
      -- happens in a different chat (incoming routing).
    ...
    -- Suppress separate executor_assigned event when it's part of the assignment
    -- transition (status_changed above already carries the message).
