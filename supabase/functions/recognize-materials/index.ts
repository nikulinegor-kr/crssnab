...
ПРАВИЛА ФОРМИРОВАНИЯ ПОЗИЦИЙ ДЛЯ ФОРМАТА 4:

1. АРМАТУРА (колонки с "Арматура класса", "A240", "A400", "A500" и т.п.):
   - name = "Изделия из арматуры {Класс} Ø{Диаметр}"
   - Примеры: "Изделия из арматуры A240 Ø8", "Изделия из арматуры A400 Ø12"
   - type_mark = ГОСТ (например "ГОСТ 34028-2016") + марка стали если указана (например "A400 сталь 25Г2С")
   - unit = "кг"

2. ТРУБЫ (колонки с "Тр.", "Труба", диаметр×толщина):
   ...
   - unit = "кг" (или "м" если указано в таблице)

3. ПРОКАТ / ПОЛОСА (колонки с "Прокат", "Полоса", ГОСТ 103):
   - name = "Прокат полоса δ{Толщина} {Марка стали}"
   - Примеры: "Прокат полоса δ6 С235", "Прокат полоса δ10 С245"
   - type_mark = ГОСТ (например "ГОСТ 103-2006") + марка стали
   - unit = "кг"

ОБЩИЕ ПРАВИЛА ДЛЯ ФОРМАТА 4:
- Каждая ячейка с числовым значением > 0 = ОТДЕЛЬНАЯ позиция материала.
- НЕ используй название строки (Водосброс, Плита и т.п.) как основное наименование — это НЕ материал.
- quantity = значение из ячейки
- mass_per_unit = null
