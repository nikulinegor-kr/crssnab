...
КРИТИЧЕСКОЕ ПРАВИЛО 1 — ФИЛЬТРАЦИЯ РАБОТ (АБСОЛЮТНЫЙ ПРИОРИТЕТ):

Каждую строку классифицируй как MATERIAL или WORK.

⛔ WORK (НИКОГДА не добавлять) — определяется по ЛЮБОМУ из условий:

УСЛОВИЕ A — НАЧАЛО СТРОКИ (приоритет №1):
Если наименование НАЧИНАЕТСЯ с любого из этих слов (регистронезависимо):
устройство, разработка, восстановление, планировка, уплотнение,
нарезка, монтаж, демонтаж, разборка, укладка, установка,
подготовка, срезка, засыпка, прокладка, пробивка, заделка,
окраска, грунтование, выравнивание, армирование, бетонирование,
расчистка, вырубка, корчёвка, снятие, удаление, очистка
→ TYPE = WORK
→ ДАЖЕ ЕСЛИ дальше в строке упоминаются материалы (бетон, щебень и т.д.)
→ ДАЖЕ ЕСЛИ строка очень длинная
Пример: "Устройство основания из щебня фр. 20-40 толщиной 200мм" → WORK
Пример: "Разработка грунта с погрузкой" → WORK

УСЛОВИЕ B — КЛЮЧЕВЫЕ СЛОВА В ЛЮБОЙ ЧАСТИ СТРОКИ:
Если строка СОДЕРЖИТ (в любом месте):
"устройство", "разработка", "планировка", "восстановление",
"уплотнение", "работы", "подготовительные работы",
"земляные работы", "монтажные работы", "строительные работы"
→ TYPE = WORK

УСЛОВИЕ C — ОПИСАНИЕ ПРОЦЕССА:
Если строка описывает действие/процесс, а не вещество:
"с уплотнением", "с послойным уплотнением", "механизированным способом"
→ TYPE = WORK

✅ MATERIAL (добавлять) — ТОЛЬКО если строка описывает ВЕЩЕСТВО/ИЗДЕЛИЕ:
- Строка НАЧИНАЕТСЯ с названия материала: щебень, песок, бетон, раствор,
  асфальтобетон, грунт, геотекстиль, геомембрана, плитка, бордюр,
  арматура, труба, профиль, лист, уголок, швеллер, краска, мастика,
  праймер, битум, эмульсия, сетка, пленка, утеплитель
- И строка НЕ содержит глаголов-действий из списка выше

ВАЖНО: Длина строки НЕ влияет на классификацию.
Строка из 100 слов, начинающаяся с "Устройство" → всё равно WORK.

ФИНАЛЬНАЯ ПРОВЕРКА (обязательна):
После формирования массива результатов, пройди по каждому элементу и проверь поле "name".
Если name содержит ЛЮБОЕ из слов: устройство, разработка, планировка, восстановление,
уплотнение, нарезка, монтаж, работы, демонтаж, укладка, установка, подготовка
→ УДАЛИ этот элемент из результата. Это работа, пропущенная фильтром.

КРИТИЧЕСКОЕ ПРАВИЛО 2 — ВЛОЖЕННЫЕ КОНСТРУКЦИИ:
...
→ извлекать ТОЛЬКО эти подпункты как материалы (применяя правило 1)

Иерархия нумерации:
- Если есть: 1, 1.1, 1.2 → строка "1" = группа (пропустить), строки "1.1", "1.2" = проверить MATERIAL/WORK
- Если нет вложенности (плоская таблица) → извлекать как есть (с фильтрацией работ)

КРИТИЧЕСКОЕ ПРАВИЛО 3 — ПОЛНЫЕ НАИМЕНОВАНИЯ (НЕ ОБРЕЗАТЬ):

Наименование материала = ВЕСЬ текст из колонки "Наименование" до колонки "Ед. изм."
- НЕ обрезать текст, даже если он длинный
- Если текст разбит на несколько строк внутри ячейки (перенос строки) → объединить в одну строку через пробел
- Слова "тип", "типа", "марка", "марки", "на битуме", "класс", "из горячей", "мелкозернистой" — это ЧАСТЬ наименования, НЕ отдельные поля
- Единицы измерения (м², м³, т, кг, шт, пог.м) НЕ должны попадать в наименование — они в отдельной колонке
- ГОСТ/ТУ/СТО если указан в конце наименования или в отдельной колонке → переносить в поле type_mark

Пример правильного извлечения:
Исходная ячейка: "Плотный асфальтобетон из горячей мелкозернистой смеси типа А, марки I, на битуме БНД 90/130"
→ name: "Плотный асфальтобетон из горячей мелкозернистой смеси типа А, марки I, на битуме БНД 90/130"
→ unit: "м³" (из соседней колонки)
→ НЕ обрезать до "Плотный асфальтобетон"!

КОНТРОЛЬ КАЧЕСТВА:
В итоговом результате НЕ ДОЛЖНО быть строк со словами "Конструкция", "Тип N", "устройство", "разработка", "планировка", "работы" в поле name.
Если есть — это ОШИБКА. Удали их.
Если наименование заканчивается на: "марки", "типа", "на", "из", "класса" — это ОШИБКА обрезки. Восстанови полный текст.\n`
        : `\n⚠️ ТИП ДОКУМЕНТА: ${typeLabel}
