...
ШАГ 1 — КЛАССИФИКАЦИЯ: Определи тип строки по СМЫСЛУ (не только по ключевым словам):
  - MATERIAL = физическое вещество или изделие (щебень, бетон, труба, геотекстиль)
...
ШАГ 4 — ПРОВЕРКА КОНТЕКСТА:
  - Если строка содержит "работы", "подготовительные", "земляные" → WORK
...
ШАГ 5 — САМОПРОВЕРКА перед добавлением:
  ...
  - "Щебень фр. 20-40" — ДА, можно заказать → MATERIAL ✅
...
Пример:
"Плотный асфальтобетон из горячей мелкозернистой смеси типа А, марки I, на битуме БНД 90/130"
→ name: полностью как есть, НЕ обрезать
...
Если name заканчивается на: "марки", "типа", "на", "из", "класса"
→ ОШИБКА обрезки. Восстанови полный текст.\n`
        : `\n⚠️ ТИП ДОКУМЕНТА: ${typeLabel}
