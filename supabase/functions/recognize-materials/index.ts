...
ПРИОРИТЕТ ТАБЛИЦ:
Если в документе есть таблица "Спецификация деталей" — она является ОСНОВНЫМ источником материалов. Обрабатывай в первую очередь её.
ИГНОРИРУЙ: чертежи, схемы, размеры на рисунках, правую часть документа (бетон, объемы, участки).
...
ФОРМАТ 4 — Сводная матричная таблица (арматура, трубы, прокат):
Это таблицы, где заголовки колонок содержат параметры (класс, диаметр, толщину), а строки — конструкции/элементы. Ячейки содержат количество (кг, м, шт и т.д.).

ФОРМАТ 5 — Спецификация деталей (чертёж с таблицей):
Колонки: "Сечение", "ГОСТ", "Длина", "Кол-во", "Масса ед.", "Масса всего"
Каждая строка = отдельный материал.
- quantity = значение из колонки "Масса всего" (общая масса)
- unit = "кг"
- mass_per_unit = значение из "Масса ед."
- type_mark = значение из колонки "ГОСТ"
- name формируется по правилам ниже на основе колонки "Сечение"
...
ПРАВИЛА ФОРМИРОВАНИЯ НАИМЕНОВАНИЙ (для ФОРМАТА 4 и ФОРМАТА 5):

1. АРМАТУРА (содержит "A240", "A400", "A500", "Ø" без "×"):
   - name = "Изделия из арматуры {Класс} Ø{Диаметр}"
   - Примеры: "Изделия из арматуры A240 Ø8", "Изделия из арматуры A400 Ø12"
   - type_mark = ГОСТ (например "ГОСТ 34028-2016") + марка стали если указана
...
2. ТРУБЫ (содержит "Тр.", "Труба", или "Ø{число}×{число}"):
   ...
   - type_mark = ГОСТ трубы (например "ГОСТ 3262-75" или "ГОСТ 10704-91") + марка стали

3. ПРОКАТ / ПОЛОСА (содержит размеры без Ø, "Прокат", "Полоса", "δ", или размеры вида "80×4"):
   - name = "Прокат полоса {Ширина}×{Толщина} {Марка стали}"
   - Примеры: "Прокат полоса 80×4 С235", "Прокат полоса δ6 С235"
   - type_mark = ГОСТ (например "ГОСТ 103-2006") + марка стали

ОБЩИЕ ПРАВИЛА ДЛЯ ФОРМАТОВ 4 и 5:
- Каждая ячейка/строка с числовым значением > 0 = ОТДЕЛЬНАЯ позиция материала.
- НЕ используй название строки (Водосброс, Плита и т.п.) как основное наименование — это НЕ материал.
- unit = "кг"
- ПОЛНОСТЬЮ ИГНОРИРУЙ колонки и строки "Итого", "Всего", "Всего по", любые суммарные.
- Если одинаковый материал встречается в нескольких строках, создавай ОТДЕЛЬНУЮ позицию для каждой ячейки (НЕ суммируй).
- Если в таблице несколько ГОСТов — привязывай каждый материал к СВОЕМУ ГОСТу по колонке/группе.
...
- Строки-заголовки подтаблиц (например «Труба 3") записывай НЕ как отдельные позиции, а добавляй как префикс к name следующих строк.
...
- "Сечение" → используй для определения name по правилам выше
