...
ФОРМАТ 4 — Сводная таблица арматуры / спецификация арматурных изделий:
Это матричная таблица, где:
- Верхний уровень заголовков = класс арматуры (A240, A400, A500, A600 и т.п.)
- Подуровень заголовков = диаметр (Ø8, Ø10, Ø12, Ø16, Ø20, Ø25 и т.п.)
- Строки = конструкции или элементы (Водосброс, Плита, Балка и т.д.)
- Ячейки = количество в кг

ПРАВИЛА ДЛЯ ФОРМАТА 4:
- Каждая ячейка с числовым значением > 0 = ОТДЕЛЬНАЯ позиция материала.
- НЕ используй название строки (Водосброс и т.п.) как основное наименование — это НЕ материал.
- Формируй name по шаблону: "Изделия из арматуры {Класс} Ø{Диаметр}"
  Примеры: "Изделия из арматуры A240 Ø8", "Изделия из арматуры A400 Ø12"
- type_mark = ГОСТ арматуры (например "ГОСТ 34028-2016") + марка стали если указана (например "A400 сталь 25Г2С")
- unit = "кг"
- quantity = значение из ячейки
- mass_per_unit = null
- ПОЛНОСТЬЮ ИГНОРИРУЙ колонки "Итого", "Всего", "Всего по" и любые суммарные колонки/строки.
- Если одинаковый класс+диаметр встречается в нескольких строках, создавай ОТДЕЛЬНУЮ позицию для каждой ячейки (НЕ суммируй).
