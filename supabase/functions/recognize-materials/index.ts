...
🔴 IGNORE (НЕ извлекать — это НЕ металлопрокат):
- Объемы работ (м³, м²) без признаков металла
- Бетон, бетонные изделия, бетонное полотно (BeNotex и т.п.)
...
ВАЖНО: Этот модуль предназначен ТОЛЬКО для металлопроката и металлоизделий:
...
5. Формат type_mark:
   ...
   - НИКОГДА не оставляй type_mark пустым
...
КРИТИЧЕСКИ ВАЖНО: type_mark НИКОГДА не должен быть null или пустым — всегда указывай ГОСТ.
